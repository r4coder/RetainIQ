...
.nav-item:hover { background: rgba(229,9,20,0.12); color: #fff; }
.nav-item.active { background: linear-gradient(135deg,rgba(229,9,20,0.25),rgba(255,107,53,0.15)); color: #fff; border-left: 3px solid #e50914; }
...
def load_models():
    artifacts = {}
    for f in ["best_model.pkl","all_results.pkl","feature_importance.pkl",
              "encoders.pkl","scaler.pkl","feature_names.pkl"]:
        p = MODELS_DIR / f
        artifacts[f.replace(".pkl","")] = joblib.load(p) if p.exists() else None
    return artifacts
...
def ensure_trained():
    ...
    if not (MODELS_DIR / "best_model.pkl").exists():
        with st.spinner("🚀 Training ML models for the first time… (30–60 s)"):
            import subprocess, sys
            subprocess.run([sys.executable, str(Path(__file__).parent.parent / "train.py")],
                           check=True)
...
def sidebar():
    with st.sidebar:
        ...
        pages = [
            ...
            ("🤖", "Models",       "ML Performance"),
            ("🔮", "Predict",      "Churn Predictor"),
            ("💡", "Retention",    "Business Intelligence"),
        ]
        ...
        st.markdown(
            ...
            '<span style="color:#e50914">v2.0</span> · ML-Powered Analytics'
            '</div>',
...
def page_overview(df):
    ...
    with c1:
        ...
        fig.update_layout(title="Churn Rate by Account Age", **PLOTLY_LAYOUT,
    ...
    with c2:
        # Donut
        vals = [churned, retained]
        lbls = ["Churned","Retained"]
        ...
        fig2.update_layout(title="Churn Composition", **PLOTLY_LAYOUT,
                           showlegend=True, legend=dict(orientation="h", y=-0.1))
        st.plotly_chart(fig2, use_container_width=True)

    # Watch hours distribution + subscription breakdown
    c3, c4 = st.columns(2)
    with c3:
        fig3 = px.histogram(
    ...
    with c4:
        ...
        fig4 = px.bar(
            sub_churn, x="Subscription", y="Churn Rate",
            color="Churn Rate",
        ...
        fig4.update_layout(**PLOTLY_LAYOUT, yaxis_tickformat=".0%", coloraxis_showscale=False)
...
def page_analytics(df):
    ...
    mask = (
        df["subscription_type"].isin(subs) &
    ...
    c1, c2 = st.columns(2)
    with c1:
        genre_churn = dff.groupby("favorite_genre")["churn"].agg(["mean","count"]).reset_index()
        genre_churn.columns = ["Genre","Churn Rate","Users"]
        genre_churn = genre_churn.sort_values("Churn Rate", ascending=True)
        fig = px.bar(genre_churn, y="Genre", x="Churn Rate", orientation="h",
                     color="Churn Rate", color_continuous_scale=["#10b981","#f59e0b","#e50914"],
                     title="Churn Rate by Favourite Genre",
                     text=[f"{v:.1%}" for v in genre_churn["Churn Rate"]])
    ...
    with c2:
        binge_dist = dff.groupby(["binge_watch_frequency","churn"]).size().reset_index(name="count")
        binge_order = ["Never","Rarely","Sometimes","Often","Always"]
        fig2 = px.bar(binge_dist, x="binge_watch_frequency", y="count", color="churn",
                      color_discrete_map={0:"#10b981",1:"#e50914"},
                      barmode="group", category_orders={"binge_watch_frequency":binge_order},
                      title="Binge Frequency vs Churn")
        fig2.update_layout(**PLOTLY_LAYOUT, xaxis_title="Binge Frequency", yaxis_title="Users",
                            legend_title="Churn")
        st.plotly_chart(fig2, use_container_width=True)
    ...
    with c3:
        ...
        fig3 = go.Figure()
        fig3.add_trace(go.Bar(name="Safe Revenue", x=rev_df["subscription_type"],
                               y=rev_df["total_rev"]-rev_df["at_risk"],
                               marker_color="#10b981", opacity=0.85))
        fig3.add_trace(go.Bar(name="Revenue at Risk", x=rev_df["subscription_type"],
                               y=rev_df["at_risk"], marker_color="#e50914", opacity=0.85))
        fig3.update_layout(barmode="stack", title="Revenue by Subscription Tier",
                           yaxis_title="Monthly Revenue (₹)", **PLOTLY_LAYOUT)
        st.plotly_chart(fig3, use_container_width=True)

    with c4:
        payment_churn = dff.groupby("payment_method")["churn"].mean().reset_index()
        payment_churn.columns = ["Method","Churn Rate"]
        fig4 = px.pie(payment_churn, values="Churn Rate", names="Method",
                      hole=0.5, title="Churn Distribution by Payment Method",
    ...
    c5,c6 = st.columns(2)
    with c5:
        fig5 = px.scatter(dff.sample(min(1000,len(dff))),
                          x="monthly_watch_hours", y="avg_session_time",
                          color=dff.sample(min(1000,len(dff)))["churn"].map({0:"Retained",1:"Churned"}),
                          color_discrete_map={"Retained":"#10b981","Churned":"#e50914"},
                          opacity=0.6, title="Watch Hours vs Avg Session Time",
                          labels={"monthly_watch_hours":"Monthly Watch Hours (h)","avg_session_time":"Avg Session (h)"})
        fig5.update_layout(**PLOTLY_LAYOUT)
        st.plotly_chart(fig5, use_container_width=True)

    with c6:
        login_bins = [0,7,14,30,60,120]
        lb_labels  = ["0-7d","8-14d","15-30d","31-60d","61-120d"]
        dff2 = dff.copy()
        dff2["login_bucket"] = pd.cut(dff2["days_since_last_login"], bins=login_bins, labels=lb_labels)
        login_grp = dff2.groupby("login_bucket", observed=True)["churn"].mean().reset_index()
        fig6 = px.line(login_grp, x="login_bucket", y="churn",
                       markers=True, title="Churn Rate by Days Since Last Login",
        ...
        fig6.update_layout(**PLOTLY_LAYOUT, yaxis_tickformat=".0%")
    ...
    fig7.update_layout(**PLOTLY_LAYOUT, coloraxis_colorbar=dict(title="r", tickfont=dict(color="#94a3b8")))
...
def page_models(arts):
    ...
    for idx,(name,res) in enumerate(results.items()):
        ...
        with cols[idx]:
            ...
            st.markdown(
                ...
                f'{"🥇 " if is_best else ""}<b style="font-family:Syne;color:#f1f5f9;font-size:0.95rem">{name}</b>'
                + ("".join([f'<div class="metric-pill" style="margin-top:0.7rem"><div class="metric-pill-label">{k}</div>'
                             f'<div class="metric-pill-value">{v:.4f}</div></div>' for k,v in m.items()]))
                + "</div>", unsafe_allow_html=True
            )
    ...
    with c2:
        ...
        fig2 = go.Figure()
        palette2 = ["#e50914","#3b82f6","#10b981","#f59e0b"]
        for (name,res),col in zip(results.items(),palette2):
            vals = [res["metrics"][m] for m in metrics_list]
            vals.append(vals[0])
            fig2.add_trace(go.Scatterpolar(
                r=vals, theta=metrics_list+[metrics_list[0]],
                fill="toself", name=name,
                line_color=col, fillcolor=col.replace("#","rgba(").replace(")",",0.07)") if "#" in col else col,
                opacity=0.85,
            ))
        fig2.update_layout(
            polar=dict(
            ...
            title="Model Performance Radar",**PLOTLY_LAYOUT,showlegend=True,
        )
        st.plotly_chart(fig2, use_container_width=True)
...
def page_predict(arts):
    ...
    feat_names = arts.get("feature_names")
...
def page_retention(df, arts):
    ...
    if model and encoders and scaler:
        ...
        X = df2[feat_order].values
        X_sc = scaler.transform(X)
        probs = model.predict_proba(X_sc)[:,1]
        df["churn_prob"] = probs
        df["risk_segment"] = pd.cut(probs,[0,0.35,0.65,1.0],labels=["Low Risk","Medium Risk","High Risk"])
    else:
    ...
    total_rev  = df["monthly_subscription_cost"].sum()
    ...
    c1,c2 = st.columns(2)
    with c1:
        seg_rev = df.groupby("risk_segment", observed=True)["monthly_subscription_cost"].sum().reset_index()
        seg_rev.columns = ["Segment","Revenue"]
        fig1 = px.pie(seg_rev, values="Revenue", names="Segment", hole=0.55,
    ...
    with c2:
        seg_sub = df.groupby(["subscription_type","risk_segment"], observed=True).size().reset_index(name="count")
        fig2 = px.bar(seg_sub, x="subscription_type", y="count", color="risk_segment",
                      color_discrete_map={"Low Risk":"#10b981","Medium Risk":"#f59e0b","High Risk":"#e50914"},
                      title="Risk Segments by Subscription Tier", barmode="group")
        fig2.update_layout(**PLOTLY_LAYOUT, xaxis_title="Subscription", yaxis_title="Customers")
        st.plotly_chart(fig2, use_container_width=True)
    ...
    for i,(icon,title,desc) in enumerate(recs):
        ...
        with col:
            st.markdown(
                f'<div style="background:linear-gradient(145deg,#111827,#0f172a);border:1px solid rgba(255,255,255,0.07);'
                f'border-radius:14px;padding:1.2rem 1.4rem;margin-bottom:0.8rem">'
